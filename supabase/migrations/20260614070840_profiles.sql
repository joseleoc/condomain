...
update on public.profiles for each row execute function public.set_current_timestamp_updated_at ();

-- Trigger en la tabla interna de Supabase Auth para automatizar la creación del perfil
drop trigger if exists trg_on_auth_user_created on auth.users;

create trigger trg_on_auth_user_created after insert on auth.users for each row execute function public.handle_new_user_signup ();
...
create policy profiles_update_owner on public.profiles for
update to authenticated using (auth.uid () = id)
with
    check (auth.uid () = id);

-- Índice parcial para buscar perfiles activos por ID. 
-- Es ultra rápido para la verificación diaria de sesiones y relaciones de datos.
create index if not exists idx_profiles_id_active on public.profiles (id)
where
    deleted_at is null;

-- Índice para búsquedas de texto (ej. si en el frontend implementas un buscador de usuarios por nombre)
-- El operador 'lower' ayuda a que las búsquedas no distingan entre mayúsculas y minúsculas.
